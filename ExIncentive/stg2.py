...
#ADDBSS STATUS TO SLAES TABLE LTE RECORDS
sql = "ALTER TABLE SALES_"+TableName+" ADD INVOICENO VARCHAR(30)"
c.execute(sql)
...
sql = "ALTER TABLE SALES_"+TableName+" ADD FINALSTATUS VARCHAR(10)"
c.execute(sql)


sql = "DECLARE    "\
"NUM               VARCHAR2(50);  "\
"INVOICENOX               VARCHAR2(50);  "\
"CHARGE               VARCHAR2(50);    "\
"PAYMENT               VARCHAR2(50);  "\
""\
"CURSOR PSTN_NO IS    "\
"SELECT EVENT_SOURCE  FROM SALES_"+TableName+" "\
"WHERE MEDIUM = 'LTE' AND COM_TYPE = 'LTE' AND BSSSTATUS <> 'TX' AND EVENT_SOURCE IS NOT NULL AND INVOICENO IS NULL;  "\
"BEGIN        "\
"OPEN PSTN_NO;      "\
""\
"LOOP               "\
...
"BEGIN  "\
"SELECT * INTO INVOICENOX , CHARGE FROM (SELECT A.INVOICE_NUM , "\
"	(A.INVOICE_NET_MNY + A.INVOICE_TAX_MNY )/1000 CHARGES "\
"	FROM GENEVA_ADMIN.BILLSUMMARY@DBLINK_GENEVA  A, SALES_"+TableName+" X "\
"	WHERE A.ACCOUNT_NUM  = X.ACCOUNT_NUM "\
"	AND BILL_DTM -1 > X.BSSDSP "\
"	AND X.PRODUCT_LABEL = NUM "\
"	ORDER BY  BILL_DTM ASC) WHERE ROWNUM < 2;     "\
"	 "\
"			SELECT SUM(PAY) INTO PAYMENT  FROM (	SELECT DISTINCT ACCOUNT_PAYMENT_DAT, ACCOUNT_PAYMENT_MNY /1000  PAY "\
"				FROM GENEVA_ADMIN.ACCOUNTPAYMENT@DBLINK_GENEVA A, SALES_"+TableName+" X  "\
"				WHERE A.ACCOUNT_NUM  = X.ACCOUNT_NUM "\
"                AND X.PRODUCT_LABEL = NUM "\
"                AND ACCOUNT_PAYMENT_DAT > X.BSSDSP -15 "\
"                AND ACCOUNT_PAYMENT_DAT < (			SELECT ACTUAL_BILL_DTM FROM (SELECT * FROM (SELECT * FROM (SELECT DISTINCT   ACTUAL_BILL_DTM "\
"	FROM GENEVA_ADMIN.BILLSUMMARY@DBLINK_GENEVA  A, SALES_"+TableName+" X "\
"	WHERE A.ACCOUNT_NUM  = X.ACCOUNT_NUM "\
"	AND ACTUAL_BILL_DTM -1 > X.BSSDSP "\
"	AND X.PRODUCT_LABEL = NUM "\
"	ORDER BY  ACTUAL_BILL_DTM ASC ) WHERE ROWNUM < 3)  ORDER BY  ACTUAL_BILL_DTM DESC ) WHERE ROWNUM < 2) "\
"				ORDER BY ACCOUNT_PAYMENT_DAT DESC);          "\
"			 "\
"				UPDATE SALES_"+TableName+" SET INVOICENO =   INVOICENOX , CHARGES =    CHARGE , TOTPAYMENT =      PAYMENT "\
"				WHERE    MEDIUM = 'LTE' AND   EVENT_SOURCE = NUM; "\
"			 "\
"EXCEPTION    WHEN OTHERS THEN   dbms_output.put_line(NUM||'  '||sqlerrm);     ROLLBACK;    	 "\
"				END;	 "\
" "\
"COMMIT;          "\
"END LOOP;     "\
"CLOSE PSTN_NO;             "\
"EXCEPTION    WHEN OTHERS THEN   dbms_output.put_line(NUM||'  '||sqlerrm);     ROLLBACK;    END;"
c.execute(sql)


sql = "UPDATE SALES_"+TableName+" B SET B.FINALSTATUS = ( "\
"SELECT distinct CASE WHEN A.TOTPAYMENT > A.CHARGES*0.8 THEN 'OK' ELSE 'XX' END "\
"FROM SALES_"+TableName+" A WHERE  A.EVENT_SOURCE= B.EVENT_SOURCE AND MEDIUM = 'LTE' AND A.COM_TYPE = B.COM_TYPE) "\
"WHERE MEDIUM = 'LTE' "
c.execute(sql)

sql = "UPDATE SALES_"+TableName+" B SET B.FINALSTATUS =  B.BSSSTATUS  WHERE B.FINALSTATUS IS NULL"
c.execute(sql)
...
"WHERE  COM_TYPE IN ('MEGALINE' , 'LTE' , 'FTTH')  AND FINALSTATUS = 'OK'  AND STATUS_STG1 = 0 AND SALES_PERSON11 = SUB_DEALER  )"\
...
sql = "UPDATE  SALES_COUNT_"+TableName+"  A SET SALES_COUNT_STG2 = (SELECT  COUNT (*) SALES_COUNT "\
"FROM  SALES_"+TableName+" "\
"WHERE  COM_TYPE IN ('MEGALINE' , 'LTE' , 'FTTH')  AND FINALSTATUS = 'OK'  AND STATUS_STG1 = 0 AND  SALES_CHANNEL1 = SUB_DEALER) "\
"WHERE SALES_COUNT_STG2 =0"
c.execute(sql)
c.execute("commit")
...
sql = "SELECT SUM(SALES_COUNT_STG2)  FROM SALES_COUNT_"+TableName+" WHERE DEALER_NAME IN ('Talentfort-Dealer','Talentfort-Franchise','Talentfort')";  
c.execute(sql)
for TABLE_NAME in c:
    SaleSum = str(TABLE_NAME[0]) 

sql = "UPDATE SALES_COUNT_"+TableName+" SET SLAB_STG2 = (SELECT SLAB FROM  "+SlabTableName+" WHERE  "+SaleSum+" BETWEEN MIN_SALES AND MAX_SALES) "\
...
" WHERE COM_TYPE = 'BB' AND FINALSTATUS = 'OK'"
c.execute(sql)
c.execute("commit")
...
" WHERE COM_TYPE = 'BB' AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'"
c.execute(sql)
c.execute("commit")
...
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) "\
" WHERE COM_TYPE = 'IPTV' AND FINALSTATUS = 'OK'"
c.execute(sql)
c.execute("commit")
...
sql = "UPDATE SALES_"+TableName+" SET COM_STG2 = (SELECT DISTINCT COMMISSION FROM "+IPTVTableName+" WHERE UPPER(PACKAGE) = UPPER(REPLACE(BB_PACKAGE_NAME,' Rental','')) "\
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) * 0.65 "\
" WHERE COM_TYPE = 'IPTV' AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'"
c.execute(sql)
c.execute("commit")
...
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) "\
" WHERE COM_TYPE = 'FTTH' AND SERO_ORDT_TYPE = 'CREATE'  AND FINALSTATUS = 'OK'"
c.execute(sql)
c.execute("commit")
...
sql = "UPDATE SALES_"+TableName+" SET COM_STG2 = (SELECT DISTINCT COMMISSION FROM "+BearerTableName+" WHERE PRODUCT = 'FTTH New' "\
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) * 0.65 "\
" WHERE COM_TYPE = 'FTTH' AND SERO_ORDT_TYPE = 'CREATE'  AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'"
c.execute(sql)
c.execute("commit")
...
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) "\
" WHERE COM_TYPE = 'FTTH' AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO'  AND FINALSTATUS = 'OK'"
c.execute(sql)
c.execute("commit")
...
sql = "UPDATE SALES_"+TableName+" SET COM_STG2 = (SELECT DISTINCT COMMISSION FROM "+BearerTableName+" WHERE PRODUCT = 'FTTH Migration' "\
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) * 0.65 "\
" WHERE COM_TYPE = 'FTTH' AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO'  AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'"
c.execute(sql)
c.execute("commit")
...
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) "\
" WHERE COM_TYPE = 'MEGALINE' AND FINALSTATUS = 'OK'" 
c.execute(sql)
c.execute("commit")
...
sql = "UPDATE SALES_"+TableName+" SET COM_STG2 = (SELECT DISTINCT COMMISSION FROM "+BearerTableName+" WHERE PRODUCT = 'Megaline'  "\
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) * 0.65 "\
" WHERE COM_TYPE = 'MEGALINE' AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'" 
c.execute(sql)
c.execute("commit")
...
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) ) "\
" WHERE COM_TYPE = 'LTE' AND FINALSTATUS = 'OK'" 
c.execute(sql)
c.execute("commit")
...
sql = "UPDATE SALES_"+TableName+" SET COM_STG2 = (SELECT DISTINCT COMMISSION FROM "+BearerTableName+" WHERE PRODUCT = 'LTE'  "\
" AND SLAB = (SELECT SLAB_STG2 FROM SALES_COUNT_"+TableName+" WHERE SUB_DEALER = SALES_CHANNEL1 OR SUB_DEALER = SALES_PERSON11 ) )  * 0.65"\
" WHERE COM_TYPE = 'LTE' AND FINALSTATUS = 'OK' AND SALES_CHANNEL1 = 'Freelancers'" 
c.execute(sql)
c.execute("commit")
...
data = pd.read_sql("SELECT DISTINCT RTOM , PRODUCT_LABEL,ACCOUNT_NUM,MEDIUM,SERO_ORDT_TYPE,COM_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11, ROUND(COM_STG1/2,2)   STAGE1_COMMISSION_PAYED,FINALSTATUS,COM_STG2 STAGE2_COMMISSION  FROM SALES_"+TableName+" WHERE SALES_CHANNEL1 = '"+Dealer+"'", engine)
...
#     data = pd.read_sql("SELECT RTOM , EVENT_SOURCE,SERO_ORDT_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11,FINALSTATUS,MEDIUM ,COM_STG1 STAGE1_COMMISSION_PAYED,COM_STG2 STAGE2_COMMISSION FROM SALES_"+TableName+" WHERE SALES_CHANNEL1 = '"+Dealer+"'", engine)
...
for SUB_DEALER in c:
    ...
    data = pd.read_sql("SELECT DISTINCT RTOM , PRODUCT_LABEL,ACCOUNT_NUM,MEDIUM,SERO_ORDT_TYPE,COM_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11, ROUND(COM_STG1/2,2)  STAGE1_COMMISSION_PAYED,FINALSTATUS,COM_STG2 STAGE2_COMMISSION  FROM SALES_"+TableName+" WHERE SALES_PERSON11 = '"+Dealer+"'", engine)
...
for DEALER_NAME in c:
    Dealer = str(DEALER_NAME[0])
    data = pd.read_sql("SELECT DISTINCT RTOM , PRODUCT_LABEL,ACCOUNT_NUM,MEDIUM,SERO_ORDT_TYPE,COM_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11, ROUND(COM_STG1/2,2) STAGE1_COMMISSION_PAYED,FINALSTATUS,COM_STG2 STAGE2_COMMISSION  FROM SALES_"+TableName+" WHERE ( SALES_CHANNEL1 = '"+Dealer+"' OR SALES_PERSON11 =  '"+Dealer+"')", engine)
    data.to_excel(TableName+"/STG2/"+Dealer.replace(".", " ")+"_"+TableName+"_STG2.xlsx")
...
for DEALER_NAME in c:
    Dealer = str(DEALER_NAME[0])
    data = pd.read_sql("SELECT RTOM , EVENT_SOURCE,SERO_ORDT_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11,FINALSTATUS,MEDIUM ,COM_STG1 STAGE1_COMMISSION_PAYED,COM_STG2 STAGE2_COMMISSION FROM SALES_"+TableName+" WHERE ( SALES_CHANNEL1 = '"+Dealer+"' OR SALES_PERSON11 =  '"+Dealer+"')", engine)
    data.to_excel(TableName+"/STG2/"+Dealer.replace(".", " ")+"_"+TableName+"_STG2.xlsx")
