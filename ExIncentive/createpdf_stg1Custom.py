...
sql = "SELECT TO_CHAR(ADD_MONTHS(TRUNC(SYSDATE,'mm'),-4),'YYYY-MON') TBNAME FROM DUAL";  
c.execute(sql)
for TBNAME in c:
...
for DEALER_NAME in c:
    ...
    sqlx = "SELECT MEDIUM , COM_TYPE ||'-'||BB_PACKAGE_NAME XX , COM_STG1 , COUNT(EVENT_SOURCE) FROM SALES_"+TableName+"  WHERE SALES_CHANNEL1 = '"+Dealer+"'  AND STATUS_STG1 = 0  AND COM_STG1 IS NOT NULL GROUP BY MEDIUM , COM_TYPE ||'-'||BB_PACKAGE_NAME,COM_STG1 ORDER BY MEDIUM , COM_TYPE ||'-'||BB_PACKAGE_NAME,COM_STG1"
    cx.execute(sqlx)
    totpay =0
    for ROW in cx:
...
sql = "SELECT A.DEALER_NAME , SUB_DEALER FROM "+DealerTableName+" A , SALES_COUNT_"+TableName+" B WHERE DEALER_TYPE in ('S','X') AND A.DEALER_NAME = B.DEALER_NAME AND PAYMRNT_STG1 IS NOT NULL"
#print(sql)
c.execute(sql)
for DEALER_NAME in c:
    Dealer = str(DEALER_NAME[0])
