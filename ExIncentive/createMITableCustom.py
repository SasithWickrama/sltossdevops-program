...
sql = "CREATE TABLE SALES_"+TableName+" AS "\
    "SELECT ZONE, REGION, PROVINCE, RTOM, CUSTOMER_REF,ACCOUNT_NUM, "\
       ...
       "0  STATUS_STG1 "\
  "FROM REPO.New_BB_BroadBand_092022_v1@DBLINK_GENEVA "\
 "WHERE     SALES_CHANNEL1 IN (SELECT DEALER_NAME FROM EX_DEALERS) "\
       "AND MEDIUM IN ('FTTH', 'COPPER') "\
       "AND SERO_ORDT_TYPE = 'CREATE' "\
"UNION "\
"SELECT ZONE, REGION, PROVINCE, RTOM, CUSTOMER_REF, ACCOUNT_NUM, PRODUCT_ID, "\
...
"SELECT ZONE, REGION,PROVINCE, RTOM,CUSTOMER_REF, ACCOUNT_NUM,PRODUCT_ID, "\
      " CUST_ORDER_NUM, SUPPLIER_ORDER_NUM,EVENT_SOURCE,SERO_ORDT_TYPE, "\
      " BILLING_CENTRE, COST_CENTRE,TARIFF_ID,TARIFF_NAME, '',PRODUCT_LABEL, "\
     "  BSSDSP,MEDIUM,SALES_CHANNEL,SALES_PERSON,'MEGALINE',0     STATUS_STG1 "\
  "FROM REPO.New_V_SLT_Voice_092022_v1@DBLINK_GENEVA "\
" WHERE     SALES_CHANNEL1 IN (SELECT DEALER_NAME FROM EX_DEALERS) "\
      " AND MEDIUM = 'COPPER' "\
      " AND SERO_ORDT_TYPE = 'CREATE' "\
"UNION "\
"SELECT ZONE, REGION, PROVINCE, RTOM,CUSTOMER_REF, ACCOUNT_NUM,PRODUCT_ID, CUST_ORDER_NUM, "\
       "SUPPLIER_ORDER_NUM, EVENT_SOURCE, SERO_ORDT_TYPE, BILLING_CENTRE,COST_CENTRE, "\
      " TARIFF_ID,TARIFF_NAME, '',PRODUCT_LABEL,BSSDSP, MEDIUM,SALES_CHANNEL,SALES_PERSON, "\
      " 'FTTH',  0     STATUS_STG1 "\
  "FROM REPO.New_V_SLT_Voice_092022_v1@DBLINK_GENEVA "\
 "WHERE     SALES_CHANNEL1 IN (SELECT DEALER_NAME FROM EX_DEALERS) "\
      " AND MEDIUM = 'FTTH' "\
...
"SELECT ZONE, REGION, PROVINCE, RTOM, CUSTOMER_REF, ACCOUNT_NUM, PRODUCT_ID, "\
  ...
  "FROM REPO.peo_n_092022_v1@DBLINK_GENEVA "\
 "WHERE     SALES_CHANNEL IN (SELECT DEALER_NAME FROM EX_DEALERS) "\
      " AND MEDIUM IN ('FTTH', 'COPPER') "\
     "  AND SERO_ORDT_TYPE = 'CREATE'"\
"UNION "\
"SELECT A.ZONE, A.REGION, A.PROVINCE, A.RTOM, A.CUSTOMER_REF, A.ACCOUNT_NUM, A.PRODUCT_ID, "\
      ...
      " FROM REPO.peo_n_092022_v1@DBLINK_GENEVA A ,SERVICE_ORDERS SO, CIRCUITS C , SERVICE_ORDER_ATTRIBUTES SOA"\
...
"UNION "\
"SELECT A.ZONE, A.REGION, A.PROVINCE, A.RTOM, A.CUSTOMER_REF,A.ACCOUNT_NUM, "\
       ...
       "0  STATUS_STG1 "\
" FROM REPO.New_BB_BroadBand_092022_v1@DBLINK_GENEVA A ,SERVICE_ORDERS SO, CIRCUITS C , SERVICE_ORDER_ATTRIBUTES SOA"\
" WHERE A.SERO_ORDT_TYPE LIKE 'CREATE-UPGRD%'"\
" AND MEDIUM = 'FTTH'"\
" AND C.CIRT_DISPLAYNAME = ACCESS_PIPE_IDENTIFIER"\
" AND C.CIRT_NAME = SO.SERO_CIRT_NAME"\
