...
for DEALER_NAME in c:
    ...
    sqlx = "SELECT MEDIUM , COM_TYPE ||'-'||CASE WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
"WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO' THEN 'UPGRADE' "\
"ELSE REPLACE(TARIFF_NAME,'LTE_','') END   XX , NVL(COM_STG1,0) , COUNT(PRODUCT_LABEL) "\
"FROM SALES_"+TableName+"  WHERE SALES_PERSON11 = '"+SubDealer+"' AND STATUS_STG1 = 0  "\
"GROUP BY MEDIUM , COM_TYPE ||'-'||CASE WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
"WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO' THEN 'UPGRADE' "\
"ELSE REPLACE(TARIFF_NAME,'LTE_','') END  ,NVL(COM_STG1,0) ORDER BY MEDIUM , "\
"COM_TYPE ||'-'||CASE WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE' THEN 'CREATE' "\
"WHEN TARIFF_NAME IS NULL AND SERO_ORDT_TYPE = 'CREATE-UPGRD SAME NO' THEN 'UPGRADE' "\
"ELSE REPLACE(TARIFF_NAME,'LTE_','') END  "
