...
sql = "SELECT PRODUCT_ID,BSSDSP,ACCOUNT_NUM,EVENT_SOURCE,PRODUCT_LABEL,SUPPLIER_ORDER_NUM FROM SALES_"+TableName+" ";  
c.execute(sql)
for REC in c:
    Dealer = str(REC[0])
    x = conn.cursor()
...
#Mobitel Dealers
Path(TableName+"/STG2/Mobitel Dealers").mkdir(parents=True, exist_ok=True)
...
data.to_excel(TableName+"/STG2/Mobitel Dealers/"+Dealer.replace(".", " ")+"_"+TableName+"_STG2.xlsx")
...
#Freelancers
Path(TableName+"/STG2/Freelancers").mkdir(parents=True, exist_ok=True)
sql = "SELECT SUB_DEALER FROM SALES_COUNT_"+TableName+" WHERE DEALER_NAME = 'Freelancers' ";  
c.execute(sql)
for SUB_DEALER in c:
    Dealer = str(SUB_DEALER[0])
    data = pd.read_sql("SELECT RTOM , EVENT_SOURCE,SERO_ORDT_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11,BSSSTATUS,MEDIUM ,COM_STG1 STAGE1_COMMISSION_PAYED,COM_STG2 STAGE2_COMMISSION  FROM SALES_"+TableName+" WHERE SALES_CHANNEL1 = '"+Dealer+"'", engine)
    data.to_excel(TableName+"/STG2/Freelancers/"+Dealer.replace(".", " ")+"_"+TableName+"_STG2.xlsx")


#Mobitel Staff
Path(TableName+"/STG2/Mobitel Staff").mkdir(parents=True, exist_ok=True)
Dealer = "Mobitel Staff"
data = pd.read_sql("SELECT RTOM , EVENT_SOURCE,SERO_ORDT_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11,BSSSTATUS,MEDIUM ,COM_STG1 STAGE1_COMMISSION_PAYED,COM_STG2 STAGE2_COMMISSION    FROM SALES_"+TableName+" WHERE SALES_CHANNEL1 = '"+Dealer+"'", engine)
data.to_excel(TableName+"/STG2/Mobitel Staff/"+Dealer.replace(".", " ")+"_"+TableName+"_STG2.xlsx")
