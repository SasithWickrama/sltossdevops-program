...
#SET MAX SLAB AS DEFAULT FOR TALENTFORT
sql = "SELECT SUM(SALES_COUNT)  FROM SALES_COUNT_"+TableName+" WHERE DEALER_NAME IN ('Talentfort-Dealer','Talentfort-Franchise','Talentfort')";  
c.execute(sql)
for TABLE_NAME in c:
    SaleSum = str(TABLE_NAME[0]) 

sql = "UPDATE SALES_COUNT_"+TableName+" SET SLAB = (SELECT SLAB FROM  "+SlabTableName+" WHERE  "+SaleSum+" BETWEEN MIN_SALES AND MAX_SALES) "\
" WHERE DEALER_NAME IN ('Talentfort-Dealer','Talentfort-Franchise','Talentfort')"
c.execute(sql)
c.execute("commit")
...
for SUB_DEALER in c:
    ...
    data = pd.read_sql("SELECT DISTINCT RTOM , PRODUCT_LABEL,ACCOUNT_NUM,MEDIUM,SERO_ORDT_TYPE,COM_TYPE,TARIFF_NAME,BSSDSP ACTIVE_DATE ,SALES_CHANNEL1 ,SALES_PERSON11 , ROUND(COM_STG1/2,2) COM_STG1  FROM SALES_"+TableName+" WHERE SALES_PERSON11 = '"+Dealer+"'", engine)
